...
parfor k = 1:nelev
    ...
    t_pc = 0; t_nm = 0; t_hj = 0; t_sb = 0; t_gd = 0; t_un = 0;
    crb_sum = 0;
    ...
    fprintf(['Elev %3.0f m | RMSE→ PCH %.2f  NM %.2f  HJ %.2f  SB %.2f  ' ...
            ...
            elev_k, RMSE_peach(k), RMSE_nm(k), RMSE_hj(k), ...
...
grid on; xlabel('Elevação (m)'); ylabel('RMSE (m)');
...
% ---------- 4. Gráficos de tempo ----------------------------------------
figure;
plot(elev_vec, [time_pch_avg; time_nm_avg; time_hj_avg; ...
                time_sb_avg;  time_gd_avg]', 'LineWidth',1.5);
grid on; xlabel('Elevação (m)'); ylabel('Tempo médio por realização (s)');
legend('PEACH','Nelder-Mead','Hooke-Jeeves','Subplex','Golden PEACH');
title(sprintf('Tempo médio de execução | %d×%d URA | SNR=%d dB', ...
      Mx, Mz, SNRdB_fixed));

% ---------- 5. Gráficos combinados (RMSE & Tempo) -----------------------
figure;
...
semilogy(elev_vec, RMSE_peach , 'x--', ...
         ...
         elev_vec, CRBth      , 'k--','LineWidth',1.5);
grid on; xlabel('Elevação (m)'); ylabel('RMSE (m)');
legend('PEACH','+ Nelder-Mead','+ Hooke-Jeeves','+ Subplex', ...
       'Golden PEACH','CRB','Location','southwest');
title('RMSE');

% (b) Tempo
subplot(1,2,2);
semilogy(elev_vec, time_pch_avg , '-x', ...
         elev_vec, time_nm_avg  , '-o', ...
         elev_vec, time_hj_avg  , '-s', ...
         elev_vec, time_sb_avg  , '-d', ...
         elev_vec, time_gd_avg  , '-^', 'LineWidth',1.5);
grid on; xlabel('Elevação (m)'); ylabel('Tempo médio por realização (s)');
legend('PEACH','Nelder-Mead','Hooke-Jeeves','Subplex','Golden PEACH', ...
       'Location','northwest');
...
% Título geral
sgtitle(sprintf('Desempenho PEACH-MUSIC – %d×%d URA | L=%d | MCS=%d | SNR=%d dB', ...
        Mx, Mz, L, MCS, SNRdB_fixed));

% Ajuste de tamanho para artigo
set(gcf,'Units','inches','Position',[0 0 7.2 4.8]);
set(gcf,'PaperPositionMode','auto');
set(findall(gcf,'-property','FontName'),'FontName','Times New Roman');
set(findall(gcf,'-property','FontSize'),'FontSize',12);

% Exporta em PDF vetorial (alta qualidade)
exportgraphics(gcf,'fig_peach_elev_rmse_tempo.pdf', ...
