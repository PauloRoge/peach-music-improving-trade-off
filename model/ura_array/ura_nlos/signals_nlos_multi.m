...
function [Yh, Yv, Y] = signals_nlos_multi(UEs, URA, lambda, N, ...
    ...
    % Para cada espalhador l:
    % d_{m,l} = ||u - s_l|| + ||s_l - p_m||
    % h_l_raw = Gamma_l * sqrt(beta_{m,l}) * exp(-j 2pi/lambda * (d_{1,l} - d_{m,l}))
    hN_raw = zeros(M, L);
    Pl_raw = zeros(L,1);
    for l = 1:L
        s_l      = scatterer_pos(l,:);
    ...
    % LoS alvo: K/(K+1) * baseline
    % NLoS total alvo: 1/(K+1) * baseline, distribuído entre L caminhos
